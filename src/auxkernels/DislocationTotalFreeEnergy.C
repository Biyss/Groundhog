...
DislocationTotalFreeEnergy::DislocationTotalFreeEnergy(const InputParameters & parameters) :
...
Real
DislocationTotalFreeEnergy::computeValue()
{
  ...
  for (unsigned int i = 0; i < _nvars; ++i)
  {
    ...
    total_energy += (*_kappas[i])[_qp] / 2.0 * crossprod.norm_sq();
  }
